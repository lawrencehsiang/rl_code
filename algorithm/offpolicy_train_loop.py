from environment.env_utils import batch_interact_environment
...
import torch
import time
from algorithm.trainer import ScamTrainer
import wandb
...
def offpolicy_train_loop(env,\
    ...
    trainer = ScamTrainer(agent=agent,\
    ...
    for i in tqdm(range(iterations)):
        ...
        if accelerator.is_main_process:
            ...
            # 这是  128，也就是说， 一次批量环境产生32段对话，四次批量共产生了128段对话
            # print("这里的trajectories是啥?", len(trajectories), trajectories[0]) 打印了第一段对话，看test.json，'trajectory_reward': -20, 'mc_return': -1.0
            ...
            # info: {'rollout.mean': -19.3671875, 'rollout.max': 0, 'rollout.min': -20}
            # print("info:", info)
            if (i+1) % eval_freq == 0:
